 ...
 */
contract PineconeFarmTimelock is AccessControlUpgradeable {
    ...
    bytes32 public constant AUTOCOMPOUNDING_ROLE = keccak256("AUTOCOMPOUNDING_ROLE");
    uint256 internal constant _DONE_TIMESTAMP = uint256(1);

    mapping(bytes32 => uint256) private _timestamps;
    uint256 public minDelay; // seconds - to be increased in production

    address payable public adminWalletAddress;
    ...
    function initialize(
        ...
        _setRoleAdmin(AUTOCOMPOUNDING_ROLE, TIMELOCK_ADMIN_ROLE);

        // deployer + self administration
        _setupRole(TIMELOCK_ADMIN_ROLE, _msgSender());
        _setupRole(TIMELOCK_ADMIN_ROLE, address(this));
        _setupRole(PROPOSER_ROLE, adminWalletAddress);
        _setupRole(EXECUTOR_ROLE, adminWalletAddress);
        _setupRole(AUTOCOMPOUNDING_ROLE, adminWalletAddress);
    ...
    function setAdminWalletAddress(address payable _adminWalletAddress) public {
        require(
            msg.sender == address(this),
            "TimelockController: caller must be timelock"
        );
        require(tx.origin == adminWalletAddress, "tx.origin != adminWalletAddress");
        adminWalletAddress = _adminWalletAddress;
    }
    ...
    function withdrawBNB() public payable {
        require(msg.sender == adminWalletAddress, "!adminWalletAddress");
        adminWalletAddress.transfer(address(this).balance);
    }

    function withdrawBEP20(address _tokenAddress) public payable {
        require(msg.sender == adminWalletAddress, "!adminWalletAddress");
        uint256 tokenBal = IERC20(_tokenAddress).balanceOf(address(this));
        IERC20(_tokenAddress).transfer(adminWalletAddress, tokenBal);
    }
    ...
    function add(
        ...
        return IPineconeFarm(_pineconefarmAddress).add(_allocPCTPoint, _want, _withUpdate, _strat);
    }
    ...
    function set(
        ...
        IPineconeFarm(_pineconefarmAddress).set(_pid, _allocPCTPoint, _withUpdate);
    }
    ...
    function setMinter(
        ...
        IPineconeFarm(_pineconefarmAddress).setMinter(_minter, _canMint);
    }

    function earn(address _stratAddress) public onlyRole(AUTOCOMPOUNDING_ROLE) {
        IPineconeStrategy(_stratAddress).earn();
    }

    function farm(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IPineconeStrategy(_stratAddress).farm();
    }

    function pause(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IPineconeStrategy(_stratAddress).pause();
    }

    function unpause(address _stratAddress) public onlyRole(EXECUTOR_ROLE) {
        IPineconeStrategy(_stratAddress).unpause();
    }
    ...
    {
        IPineconeFarm(_pineconefarmAddress).inCaseTokensGetStuck(_token, _amount);
    }

    function inCaseTokensGetStuckStrat(
        address _stratAddress, 
        address _token, 
        uint256 _amount
    ) public onlyRole(EXECUTOR_ROLE)
    {
        IPineconeStrategy(_stratAddress).inCaseTokensGetStuck(_token, _amount);
    }
    ...
    {
        IPineconeFarm(_pineconefarmAddress).setCakeRewardsPid(_cakeRewardsPid);
    }

    function setPctPerBlock(
        address _pineconefarmAddress,
        uint256 _PCTPerBlock, 
        uint256 _startBlock
    ) public onlyRole(EXECUTOR_ROLE)
    {
        IPineconeFarm(_pineconefarmAddress).setPctPerBlock(_PCTPerBlock, _startBlock);
    }

    function setWithdrawFeeFactor(
        address _stratAddress,
